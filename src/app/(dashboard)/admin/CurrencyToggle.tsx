...
export function CurrencyToggle({ currency }: { currency: CurrencyRow }) {
  ...
  const toggle = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/currencies/${encodeURIComponent(currency.code)}`, {
        method: "PATCH",
  ...
  return (
    ...
    >
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <span className="mono" style={{ fontSize: 12, minWidth: 36 }}>{currency.code}</span>
        <span className="ui muted" style={{ fontSize: 12 }}>{currency.label}</span>
