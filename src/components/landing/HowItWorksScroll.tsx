...
const STEPS = [
  ...
  },
  {
    ...
    copy: "A form that knows 32 exchanges and 8 currencies, or one CSV import.",
  },
  {
    n: "3",
    ...
    <div className="w-full max-w-[340px] rounded-2xl border border-subtle bg-surface p-6 shadow-card">
      ...
      <div className="mt-5 space-y-3">
        <div className="flex items-center gap-2 rounded-[10px] border border-subtle bg-elevated px-3 py-2.5 font-ui text-[12px] text-secondary">
          <Icon name="landmark" size={14} className="text-muted" />{" "}
          you@example.com
        </div>
  ...
  return (
    <div className="w-full max-w-[380px] rounded-2xl border border-subtle bg-surface p-5 shadow-card">
      ...
      {rows.map(([a, b, c], i) => (
        <div
          key={i}
          className="grid grid-cols-3 gap-2 border-b border-subtle py-2.5 font-mono text-[12px] last:border-0"
        >
...
function MockInsights() {
  return (
    <div className="w-full max-w-[380px] rounded-2xl border border-subtle bg-surface p-5 shadow-card">
      ...
      <div className="mt-1 font-mono text-[22px] font-semibold tabular-nums text-primary">
        S$1,248,930
      </div>
      <div className="mt-3">
        <Spark
          pts={[3, 4, 3.4, 4.6, 4.2, 5.4, 5, 6.2, 5.8, 7]}
          color="var(--gold)"
          w={330}
          h={70}
        />
      ...
      <div className="mt-3 flex gap-2">
        <span className="rounded-full bg-gain/10 px-2.5 py-0.5 font-mono text-[11px] font-semibold text-gain">
          +1.2% today
        </span>
        <span className="rounded-full bg-fx-up/10 px-2.5 py-0.5 font-mono text-[11px] font-semibold text-fx-up">
          FX +S$12,480
        </span>
      </div>
    </div>
        ...
        <div className="mt-12 grid gap-12 md:grid-cols-3">
          {STEPS.map((s, i) => (
            <div key={s.n}>
              ...
              <h3 className="mt-4 font-ui text-[16px] font-semibold text-primary">
                {s.title}
              </h3>
              <p className="mt-2 font-ui text-[13.5px] leading-relaxed text-secondary">
                {s.copy}
              </p>
              <div
                aria-hidden
                className="pointer-events-none mt-6 grid select-none place-items-center"
              >
                {MOCKS[i]}
              </div>
            </div>
    ...
    <section ref={ref} className="relative h-[300vh]">
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        <div className="mx-auto grid w-full max-w-[1140px] items-center gap-10 px-6 md:grid-cols-2">
            ...
            <ol className="mt-10 space-y-7">
              {STEPS.map((s, i) => (
                <li key={s.n} className="flex gap-4">
                  ...
                  <motion.div
                    animate={{ opacity: active === i ? 1 : 0.62 }}
                    transition={{ duration: 0.3 }}
                  >
                    <h3 className="font-ui text-[17px] font-semibold text-primary">
                      {s.title}
                    </h3>
                    <p className="mt-1 max-w-[42ch] font-ui text-[13.5px] leading-relaxed text-secondary">
                      {s.copy}
                    </p>
                  </motion.div>
                </li>
          ...
          <div
            aria-hidden
            className="pointer-events-none relative h-[340px] select-none max-bp768:hidden"
          >
            {MOCKS.map((m, i) => (
              <Layer key={i} p={scrollYProgress} index={i} count={MOCKS.length}>
                {m}
              </Layer>
            ))}
          </div>
  ...
  return (
    <div
      className={
        ...
        (active
          ? "border-gold-soft text-gold [box-shadow:0_0_18px_var(--accent-glow)]"
