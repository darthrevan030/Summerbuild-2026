...
export function Dumbbell({ asset, fx, scale }: DumbbellProps) {
  ...
  return (
    <div className="relative h-[18px] w-full max-bp380:hidden">
      ...
      <div
        className="absolute top-[7px] h-1 rounded-[3px]"
        style={{
          left: aLeft + "%",
          width: aWidth + "%",
      ...
      />
      <div
        className="absolute top-1 h-2.5 rounded-[3px] opacity-95"
        style={{
          left: fxFrom + "%",
          width: fxWidth + "%",
          background: fxColor,
      ...
      />
      <div
        className="absolute top-[3px] h-3 w-0 -translate-x-px border-l-2"
        style={{
          left: 50 + totalEnd + "%",
          borderColor: total >= 0 ? assetColor : "var(--loss)",
        }}
      />
    </div>
  );
